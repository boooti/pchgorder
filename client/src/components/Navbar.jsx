      ...
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/80 transition-all shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 sm:h-20 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
              ...
              >
                ...
                <div className="block">
                  <div className="text-xs font-black text-slate-900 leading-tight group-hover:text-blue-700 transition-colors">
                    <span className="sm:hidden">{currentUser.name.split(' ').slice(-1)[0]}</span>
                    <span className="hidden sm:inline">{currentUser.name}</span>
                  </div>
                  <div className="text-[10px] text-slate-500 font-medium leading-none mt-0.5 hidden sm:block">
                    {currentUser.department || 'Nhân viên'}
                  </div>
                </div>
                <div className="px-1.5 sm:px-2 py-0.5 rounded-lg bg-blue-600 text-white text-[10px] font-bold flex items-center gap-1 group-hover:bg-blue-700 transition-colors shrink-0">
            ...
            {/* Admin Switcher - Always require password (hidden on mobile, accessible via bottom bar) */}
            <button
              onClick={onOpenAdminLogin}
              className={`hidden sm:flex py-2 px-3 sm:px-3.5 rounded-2xl text-xs font-bold transition-all items-center gap-1.5 border ${
                activeView === 'admin'
