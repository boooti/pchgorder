  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl overflow-hidden border border-slate-200 flex flex-col max-h-[96vh] transition-all">
        ...
        {modalMode === 'list' && (
          <>
            {/* Header - Modern Executive PCHG Style */}
            <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-950 via-blue-950 to-slate-900 text-white relative shrink-0 border-b border-white/10">
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-0.5">
                      47 thành viên thuộc 6 phòng ban Phú Cường Hoàng Gia
                    </p>
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 pt-1">
                ...
                <div className="relative flex-1 max-w-md">
                  ...
                  <input
                    type="text"
                    placeholder="Tìm theo tên, chữ cái, hoặc phòng ban..."
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    autoFocus
                    className="w-full pl-9 pr-8 py-2 bg-white/10 text-white placeholder-slate-400 rounded-xl text-xs border border-white/20 focus:outline-none focus:ring-2 focus:ring-blue-400/50 backdrop-blur-md transition-all"
                  />
                  {search && (
                    <button
                      onClick={() => setSearch('')}
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none text-[11px]">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl font-bold transition-all whitespace-nowrap ${
                      selectedDept === 'ALL'
                        ? 'bg-blue-600 text-white shadow-md ring-1 ring-white/20'
                  ...
                  {departments.map((d) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl transition-all whitespace-nowrap ${
                        selectedDept === d
                          ? 'bg-blue-600 text-white shadow-md font-bold ring-1 ring-white/20'
                  ...
                  {filtered.map((emp) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative p-2.5 sm:p-3 rounded-2xl border text-left transition-all flex items-center justify-between gap-2.5 cursor-pointer ${
                          isSelected
                            ? 'bg-blue-50/95 border-blue-500 ring-2 ring-blue-500/30 shadow-md scale-[1.01]'
                            : 'bg-white border-slate-200/90 hover:border-blue-400 hover:shadow-md hover:-translate-y-0.5 shadow-2xs'
                        }`}
                      >
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                          {/* Monogram Gradient Avatar */}
                          <div
                            className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center font-black text-xs sm:text-sm shrink-0 ${getAvatarBadgeClass(
                              emp.name
                            )}`}
                          >
                            {initialLetter}
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1">
                              <span
                                className={`inline-block text-[9px] px-1.5 py-0.5 rounded-md border truncate max-w-full ${getDeptTagStyle(
                                  emp.department
                                )}`}
                              >
                        ...
                        <div className="shrink-0 flex items-center gap-1">
                          {emp.has_password ? (
                            <span
                              ...
                              className="text-amber-600 bg-amber-50 p-1 rounded-lg border border-amber-200"
                            >
